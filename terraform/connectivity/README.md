...
Two firewall metric alerts (health state, SNAT port utilization), gated on
`firewall_enabled` exactly like the firewall itself — plain `count`, so unlike a
policy-deployed alert these are destroyed along with the firewall on toggle-off,
no orphaned-alert cleanup needed. They notify through `management`'s shared
action group (`management_workload` + `alert_emails` must resolve there first).
...
| [azurerm_monitor_metric_alert.firewall_snat_port_utilization](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/monitor_metric_alert) | resource |
...
| <a name="input_management_workload"></a> [management\_workload](#input\_management\_workload) | The management component's workload name. Used to locate its shared action group for the firewall alerts below — see the naming contract in the repo README. | `string` | `"mgmt"` | no |
